...
**סדרה (ממשית)** היא אוסף **אינסופי** של מספרים ממשיים, שבו יש **חשיבות לסדר** ו**חזרות מותרות** (ראו את הדיון על ההבדל בין סדרה לקבוצה [בפרק המבוא](06-sequences-intro.qmd)): $$a_0, a_1, a_2, \dots, a_n, \dots$$ $a_n$ הוא ה**איבר** שבמקום $n$, ו-$n$ הוא ה**אינדקס** הרץ על המספרים הטבעיים. את הסדרה כולה נסמן בקיצור $\{a_n\}_{n=0}^{\infty}$.
:::

::: {.callout-note collapse="true" title="ההגדרה הפורמלית: סדרה כפונקציה"}
פורמלית, סדרה ממשית $\{a_n\}_{n=0}^{\infty}$ היא **פונקציה** $a:\mathbb{N}\to\mathbb{R}$ — לכל מספר טבעי $n$ מותאם ערך ממשי יחיד $a_n = a(n)$. הצגה זו הופכת את ״הסדר״ ואת ״האינסופיות״ למדויקים. אין הכרח להתחיל את האינדקס ב-$0$: סדרה $\{a_n\}_{n=1}^{\infty}$ היא פונקציה מ-$\{1,2,3,\dots\}$, וכן הלאה — נשתמש בשני הסימונים לפי הנוחות.
:::

הנה כמה דוגמאות:

::: {#box-exm-sequences .thmexm}
- $a_n = n$ נותנת $0, 1, 2, 3, \dots$
- $a_n = (-1)^n$ נותנת $1, -1, 1, -1, \dots$
- הסדרה הקבועה $a_n = 8$ נותנת $8, 8, 8, 8, \dots$
- אפשר להגדיר סדרה גם [**ברקורסיה**](05-induction.qmd#sec-recursive-definition): $a_0 = 1$ ו-$a_{n+1} = (n+1)\,a_n$ נותנת $1, 1, 2, 6, 24, \dots$ . כלומר $a_n = n!$.
:::
...
תהי $\{a_n\}_{n=0}^{\infty}$ סדרה. נאמר שהסדרה **מתכנסת**, או שיש לה **גבול**, אם קיים $L\in\mathbb{R}$ כך שלכל $\varepsilon>0$ קיים $N\in\mathbb{N}$ כך שלכל $n > N$ מתקיים $$|a_n - L| < \varepsilon$$ במקרה זה נאמר שהסדרה **שואפת** (או **מתכנסת**) ל-$L$, ונסמן $\lim\limits_{n\to\infty}a_n=L$ או $a_n \to L$. אם אין לסדרה גבול, נאמר שהיא **מתבדרת**.
:::
