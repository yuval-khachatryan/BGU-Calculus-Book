...
::: {#box-prp-double-inclusion .thmprp}
...
נחזור לעניין תיאור איבריה של קבוצה. אף על פי שהסימון בעזרת סוגריים מסולסלים הוא פשוט וחד-משמעי, הוא אינו תמיד נוח, ולעיתים אף אינו מספיק. כל עוד באוסף יש מעט איברים אין בכך בעיה, אך מה נעשה אם במקום 3 איברים יש 100,000? כתיבת כולם עלולה להיות מייגעת למדי. **הכלל הוא שסימון נועד להקל על ההבנה ועל הניתוח של המידע, לא לסרבל אותם**; **אם סימון מסרבל במקום לפשט — אין בו צורך.** בעיה שנייה, חמורה אף יותר, מתעוררת כאשר בקבוצה יש אינסוף איברים — למשל אוסף כל המספרים הטבעיים $1,2,3,\dots$. במקרה כזה הסימון שהצגנו אינו מספיק כלל, שכן לא ניתן לכתוב אינסוף איברים בתוך סוגריים מסולסלים. לכן נזדקק לדרכים נוספות לתיאור קבוצות.

מהן האפשרויות? כאשר ההקשר ברור, לעיתים אפשר שלא לרשום את כל האיברים אלא להסתפק בכמה מהם ובשלוש נקודות. למשל, את קבוצת המספרים הטבעיים, המוכרת לכולכם, נהוג לכתוב $\mathbb{N}=\{1, 2, 3, \dots\}$; שלושת האיברים הראשונים $1, 2, 3$ כבר מבהירים את התבנית, ושלוש הנקודות מציינות שהרשימה נמשכת באותו אופן. (בקבוצה זו נדון בהרחבה [בסעיף על מערכות מספרים](03-number-systems.qmd#sec-number-systems).) דרך שנייה, נוחה לעיתים יותר — בפרט להגדרת קבוצות ותת-קבוצות באמצעות תכונה או נוסחה — היא **שיטת השומר** (סימון בונה-קבוצות), שאותה נציג כעת.

::: {#box-def-set-builder .thmdef}
...
- $\forall n \in \mathbb{N},\ n \ge 1$ — אמת, שכן כל מספר טבעי הוא חיובי;
...
לעיתים נתקלים בקבוצה שאין בה אף איבר. למשל, קבוצת כל המספרים הטבעיים הקטנים מ-$1$ ריקה, וכן קבוצת הפתרונות הממשיים של המשוואה $x^2 = -1$. קבוצה כזו נקראת ה**קבוצה הריקה**.

::: {#box-def-empty-set .thmdef}
...
ax.set_aspect("equal")
ax.axis("off")
fig.savefig("figures/c02_fig01.png", dpi=150, bbox_inches="tight")
plt.close(fig)
```

```{=latex}
\par\medskip
\noindent\beginL\hbox to \linewidth{\hss\includegraphics[width=0.5\linewidth]{figures/c02_fig01.png}\hss}\endL\par
\medskip
```
...
![דיאגרמת ון: האיחוד $A \cup B$ הוא האזור המוצלל.](figures/c02_fig01.png){#fig-union width="50%" fig-align="center"}
:::

::: {#box-exm-union .thmexm}
עבור $A = \{1,2,3,4\}$ ו-$B = \{3,4,5,6\}$ מתקבל $$.A \cup B = \{1,2,3,4,5,6\}$$
:::

### חיתוך

::: {#box-def-intersection .thmdef}
...
ax.set_aspect("equal")
ax.axis("off")
fig.savefig("figures/c02_fig02.png", dpi=150, bbox_inches="tight")
plt.close(fig)
```

```{=latex}
\par\medskip
\noindent\beginL\hbox to \linewidth{\hss\includegraphics[width=0.5\linewidth]{figures/c02_fig02.png}\hss}\endL\par
\medskip
```
...
::: {.content-visible when-format="html"}
![דיאגרמת ון: החיתוך $A \cap B$ הוא האזור המשותף המוצלל.](figures/c02_fig02.png){#fig-intersection width="50%" fig-align="center"}
:::

::: {#box-exm-intersection .thmexm}
עבור אותן $A = \{1,2,3,4\}$ ו-$B = \{3,4,5,6\}$ מן הדוגמה הקודמת, $$.A \cap B = \{3,4\}$$ שכן רק $3$ ו-$4$ שייכים לשתי הקבוצות.
:::

### הפרש

::: {#box-def-difference .thmdef}
...
ax.set_aspect("equal")
ax.axis("off")
fig.savefig("figures/c02_fig03.png", dpi=150, bbox_inches="tight")
plt.close(fig)
```

```{=latex}
\par\medskip
\noindent\beginL\hbox to \linewidth{\hss\includegraphics[width=0.5\linewidth]{figures/c02_fig03.png}\hss}\endL\par
\medskip
```
...
::: {.content-visible when-format="html"}
![דיאגרמת ון: ההפרש $A \setminus B$ הוא החלק של $A$ שאינו ב-$B$ (האזור המוצלל).](figures/c02_fig03.png){#fig-difference width="50%" fig-align="center"}
:::

::: {#box-exm-difference .thmexm}
עבור $A = \{1,2,3,4\}$ ו-$B = \{3,4,5,6\}$ מתקבל $$.A \setminus B = \{1,2\}$$ ואילו $$.B \setminus A = \{5,6\}$$ ואכן שני ההפרשים שונים זה מזה.
:::

### הפרש סימטרי

::: {#box-def-symmetric-difference .thmdef}
...
ax.set_aspect("equal")
ax.axis("off")
fig.savefig("figures/c02_fig04.png", dpi=150, bbox_inches="tight")
plt.close(fig)
```

```{=latex}
\par\medskip
\noindent\beginL\hbox to \linewidth{\hss\includegraphics[width=0.5\linewidth]{figures/c02_fig04.png}\hss}\endL\par
\medskip
```
...
::: {.content-visible when-format="html"}
![דיאגרמת ון: ההפרש הסימטרי $A \triangle B$ הוא כל מה ששייך לאחת הקבוצות בלבד (האזור המוצלל).](figures/c02_fig04.png){#fig-symmetric-difference width="50%" fig-align="center"}
:::

::: {#box-exm-symmetric-difference .thmexm}
...
::: {#box-prp-symmetric-difference .thmprp}
לכל שתי קבוצות $A$ ו-$B$ מתקיים $$.A \triangle B = (A \cup B) \setminus (A \cap B)$$
:::

::: {.thmproof}
נוכיח את השוויון ב**שיטת ההכלה הכפולה**: נראה שכל אחת מן הקבוצות מוכלת בשנייה.
...
ax.set_aspect("equal")
ax.axis("off")
fig.savefig("figures/c02_fig05.png", dpi=150, bbox_inches="tight")
plt.close(fig)
```

```{=latex}
\par\medskip
\noindent\beginL\hbox to \linewidth{\hss\includegraphics[width=0.5\linewidth]{figures/c02_fig05.png}\hss}\endL\par
\medskip
```
...
::: {.content-visible when-format="html"}
![דיאגרמת ון: המשלים $A^{c}$ הוא כל מה שבתוך העולם $U$ ומחוץ ל-$A$ (האזור המוצלל).](figures/c02_fig05.png){#fig-complement width="50%" fig-align="center"}
:::

::: {#box-exm-complement .thmexm}
...
::: {#box-prp-difference-complement .thmprp}
תהי $U$ קבוצה אוניברסלית ו-$A, B \subseteq U$. אז $$.A \setminus B = A \cap B^{c}$$
:::

::: {.thmproof}
כאן כל צעד הוא שקילות, ולכן נוכיח את השוויון בשרשרת שקילויות אחת (במקום בשתי הכלות נפרדות): לכל $x$, $$.x \in A \setminus B \iff (x \in A \ \wedge\ x \notin B) \iff (x \in A \ \wedge\ x \in B^{c}) \iff x \in A \cap B^{c}$$ השקילות הראשונה היא הגדרת ההפרש, השנייה — הגדרת המשלים, והשלישית — הגדרת החיתוך. מאחר שלכל $x$ מתקיים $x \in A \setminus B$ אם ורק אם $x \in A \cap B^{c}$, שתי הקבוצות שוות.
:::
